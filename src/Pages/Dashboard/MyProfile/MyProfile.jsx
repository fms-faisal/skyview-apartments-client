...
const MyProfile = () => {
  ...
  // Get reservations based on user email
  const userReservations = reservation.filter(res => res.email === user.email);

  // Check if the user is an admin or has reservations
  const [isAdmin] = useAdmin();
  const [isMember] = useMember();

  // Set profile details
  const profileDetails = isAdmin || isMember ? userReservations[0] : {
    apartment_no: "none",
    block_name: "none",
    floor_no: "none",
    rent: "none"
    ...
    <div className="flex justify-center items-center mt-16 h-full">
      <div className="w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
        ...
        <div className="flex items-center px-6 py-3 bg-gray-900">
          ...
          <h1 className="mx-3 text-lg font-semibold text-white">Apartment No: {profileDetails.apartment_no}</h1>
        </div>
        <div className="px-6 py-4">
          <h1 className="text-xl font-semibold text-gray-800 dark:text-white">{user.displayName || "None"}</h1>
          <p className="py-2 text-gray-700 dark:text-gray-400">{user.email || "None"}</p>
          <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
            ...
            <h1 className="px-2 text-sm">Block: {profileDetails.block_name}</h1>
          </div>
          <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
            ...
            <h1 className="px-2 text-sm">Floor: {profileDetails.floor_no}</h1>
          </div>
          <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
            ...
            <h1 className="px-2 text-sm">Rent: {profileDetails.rent}</h1>
          </div>
          <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
            <svg aria-label="email icon" className="w-6 h-6 fill-current" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
            ...
            <h1 className="px-2 text-sm">Agreement data: {profileDetails.requested_date ? new Date(profileDetails.requested_date).toLocaleDateString() : "N/A"}</h1>
          </div>
        </div>
      </div>
    </div>
